  ...
  function applyPublicTemplatePatches() {
    try {
      ...
      bookingMessage = function () {
        ...
        const courtFeeText = courtCfg.included ? "Included in coaching rate" : (courtCfg.bookingMessage || courtCfg.detail || "Not included; confirmed separately");
      ...
      loadPublicWeekPreview = async function () {
        ...
        for (let h = startHour; h < endHour; h++) {
          ...
          for (const d of days) {
            ...
            b.textContent = label;
            if (clickable) { b.type = "button"; b.onclick = () => jumpFromWeekToBooking(ds, h); }
      ...
      loadLiveAvailabilityIndicator = async function () {
        ...
        if (error) { badge.querySelector("strong").textContent = "Live schedule available below"; return; }
        ...
        if (open === 0) { badge.classList.add("full"); badge.querySelector("strong").textContent = "No open hours left this week"; }
        else if (open <= 5) { badge.classList.add("limited"); badge.querySelector("strong").textContent = `Limited availability • ${open} open hour${open === 1 ? "" : "s"} this week`; }
        else { badge.querySelector("strong").textContent = todayOpen ? `${todayOpen} open hour${todayOpen === 1 ? "" : "s"} today • ${open} this week` : `${open} open coaching hours this week`; }
      };

(() => {
  ...
  const dynamicReplacements = () => [
    ...
    ["PICKYLA", String(brand.shortName || brand.name || "Coach Booking").toUpperCase()],
    ["Pickyla", brand.name || "Your Coaching Brand"],
    ["Kyla", coach.firstName || "Coach"],
    ["Your City, Your Province", location.full || "Your City, Your Province"],
    ["Santiago City", location.city || "Your City"]
  ];

  const replaceString = value => {
    let text = String(value ?? "");
    dynamicReplacements().forEach(([from, to]) => { text = text.split(from).join(to); });
    return text;
  };

  if (window.CanvasRenderingContext2D && !CanvasRenderingContext2D.prototype.__coachTemplateFillTextPatched) {
    const realFillText = CanvasRenderingContext2D.prototype.fillText;
    CanvasRenderingContext2D.prototype.fillText = function (text, ...args) {
      return realFillText.call(this, replaceString(text), ...args);
    };
    Object.defineProperty(CanvasRenderingContext2D.prototype, "__coachTemplateFillTextPatched", { value: true });
  }
  ...
  function applyStaticConfig() {
    ...
    if (quick[0]) { const s = quick[0].querySelector("span"); if (s) s.innerHTML = `<strong>${location.city || "Your City"}</strong><small>${location.province || "Your Province"}</small>`; }
    if (quick[1]) { const s = quick[1].querySelector("span"); if (s) s.innerHTML = `<strong>${schedule.hoursLabel || "8 AM – 12 MN"}</strong><small>${schedule.daysLabel || "Monday–Sunday"}</small>`; }
    if (quick[2]) { const s = quick[2].querySelector("span"); if (s) s.innerHTML = `<strong>${schedule.durationLabel || "Flexible Duration"}</strong><small>${schedule.durationNote || "Choose consecutive available hours"}</small>`; }
    ...
    packageButtons.forEach((button, index) => {
      ...
      button.innerHTML = index === 0 ? `1-on-1 <small>${money(rate.amount)}/hr</small>` : `${rate.label} <small>${range} players • ${money(rate.amount)} each</small>`;
    });
    ...
    if (facebook) { facebook.href = contact.facebookUrl || "#"; facebook.textContent = contact.facebookLabel || "Facebook / Messenger"; }
  ...
  function startObserver() {
    const observer = new MutationObserver(records => { records.forEach(record => record.addedNodes.forEach(node => replaceTextInNode(node))); });
    observer.observe(document.body, { childList: true, subtree: true });
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", () => { applyStaticConfig(); replaceTextInNode(document.body); startObserver(); });
  } else {
    applyStaticConfig(); replaceTextInNode(document.body); startObserver();
  }

  window.CoachTemplate = { config: cfg, money, applyStaticConfig, replaceString };
})();

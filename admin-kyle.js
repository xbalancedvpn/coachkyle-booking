...
function bookingCard(b,todayMode=false){const paid=paymentTotals.get(b.id)||0,total=Number(b.total_amount||0),bal=Math.max(0,total-paid),payState=bal<=0?'Paid':paid>0?'Partial':'Unpaid';return `<article class="card"><div class="card-top"><div><h3>${esc(b.client_name)}</h3><div class="meta">${esc(b.session_date)} • ${hour(b.start_hour)}–${hour(b.end_hour)}<br>${b.participant_count} player${b.participant_count>1?'s':''} • ${money(total)}<br>${esc(b.contact||'No contact')}</div></div><div class="tag-stack"><span class="tag">${esc(b.session_status||'scheduled')}</span><span class="tag payment ${payState.toLowerCase()}">${payState}${bal?` • ${money(bal)} due`:''}</span></div></div><div class="card-actions">${todayMode&&b.session_status!=='completed'?`<button class="mini confirm" data-complete="${b.id}">Mark Completed</button>`:''}<button class="mini" data-payment="${b.id}">Record Payment</button><button class="mini confirm-card" data-confirmation="${b.id}">Confirmation PNG</button><button class="mini danger" data-cancel-booking="${b.id}">Cancel Booking</button></div></article>`}
...
async function setSessionStatus(b,status){if(status==='completed'&&!confirm(`Mark ${b.client_name}'s session as completed?`))return;const {error}=await db.from('bookings').update({session_status:status,session_closed_at:status==='completed'?new Date().toISOString():null}).eq('id',b.id);if(error)return toast(error.message);toast('Session marked completed.');loadBookings();loadClients()}

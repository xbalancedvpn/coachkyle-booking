// Coach Booking Template v17-F public overlay
...
(function(){
  ...
  window.coachBookingShowPublicTab=showFocusedView;
  ...
  if(sendRequestButton){
    sendRequestButton.onclick=async()=>{
      ...
      try{
        ...
        const {error}=await db.rpc('submit_public_inquiry_v17f',{
        ...
        const cfg=window.COACH_APP_CONFIG||{},brand=cfg.brand?.name||'the coaching team',coach=cfg.coach?.firstName||'the coach';
        requestSubmitStatus.className='request-submit-status ok';
        requestSubmitStatus.textContent=selfAssessment?
          `Request sent to ${brand} Admin. Your initial self-assessment was saved as a starting baseline and will be linked after ${coach} confirms the booking.`:
          `Request sent to ${brand} Admin. Your slot is still subject to final confirmation by ${coach}.`;
      }catch(e){
